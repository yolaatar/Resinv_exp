#!/usr/bin/env python3
"""
Prepare TEM1 + TEM2 dataset for nnUNet training — full multi-resolution model, v2
(nnUNet 2.8.1 retrain).

Same as prepare_dataset_multires_full.py (model 5, Dataset005, nnunetv2==2.2.1), but writes
to a new dataset ID/name so the original 2.2.1-trained model and its preprocessed data are
left untouched for comparison.

Differences from prepare_dataset_multires_v2.py (TEM1-only model):
...
Trained on fold_all (100% of cases, no internal val split), so no custom split is needed
here unlike the TEM1-only v2 model.

Usage (on cluster):
    python prepare_dataset_multires_full_v2.py \
...
def main():
    ...
    print("======================================================")
    print(f" Full multi-resolution dataset preparation (v2)")
    ...
    dataset_json = {
        ...
        "description": (
            ...
            f"No held-out test split — both datasets used fully for training, fold_all."
        ),
    }
    ...
    print(f"  CUDA_VISIBLE_DEVICES=1 nnUNetv2_train {DATASET_ID} 2d all")
