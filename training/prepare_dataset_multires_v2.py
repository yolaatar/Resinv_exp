#!/usr/bin/env python3
"""
Prepare TEM1 dataset for nnUNet training — multi-resolution model, v2 (nnUNet 2.8.1 retrain).

Same as prepare_dataset_multires.py (model 2, Dataset002, nnunetv2==2.2.1), but writes to a
new dataset ID/name so the original 2.2.1-trained model and its preprocessed data are left
...
fold_0 for this dataset is trained on a custom, group-aware split (see
generate_splits_multires.py) that groups all resolution-duplicates of a source image
together, so nnUNet's internal train/val split doesn't leak near-identical content
across resolutions and stays balanced across pixel sizes.

Usage (on cluster):
    python prepare_dataset_multires_v2.py \
...
def main():
    ...
    dataset_json = {
        ...
        "name": DATASET_NAME,
        "description": (
            f"TEM1 multi-resolution training dataset (v2, nnUNet 2.8.1 retrain). "
            f"Original ({ORIGINAL_PX} μm/px) + downsampled at {EXTRA_PX} μm/px. "
            f"Same 80/20 subject split as Dataset001_TEM_witness. "
            f"fold_0 uses a custom group-aware split, see generate_splits_multires.py."
        ),
    }
    ...
    print(f"  python generate_splits_multires.py --nnunet-raw <raw> --nnunet-preprocessed <preprocessed> --dataset-name {DATASET_NAME}")
